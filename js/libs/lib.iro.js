	  ...
	  IroComponent.prototype.handleEvent = function handleEvent (e) {
	    ...
	    switch (e.type) {
	      ...
	      case EVENT_TOUCHSTART:
					if (this.handleInput(x, y, bounds, 'START') || $(e.target).hasClass('iro__slider__value')) { // keep within bounds of circle, always work for sliders
	...
	var IroWheel = /*@__PURE__*/(function (IroComponent$$1) {
	  ...
	  */
	  IroWheel.prototype.handleInput = function handleInput (x, y, ref, type) {
	    ...
	    var handleDist = Math.sqrt(x * x + y * y);
			if (type == 'START' && handleDist > radius) {
				// console.warn("Handle Dist Exit", type, handleDist, radius);
				return false; // don't listen nor prevent event
			}

	    // Calculate the hue by converting the angle to radians
	    var hue = this._transformAngle(Math.round(handleAngle * (180 / Math.PI)) + 180);

			// make sure dist is max of range
			handleDist = Math.min(handleDist, handleRange);
	    ...
	    props.onInput(type, {
	      ...
	      s: Math.round((100 / handleRange) * handleDist)
			...
			return true; // success, prevent other touch/mouse events
	  };
